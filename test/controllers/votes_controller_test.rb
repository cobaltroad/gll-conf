...
class VotesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "current user votes on a new question" do
    ...
    post vote_api_question_url(id: question.id, yes_vote: false), headers: @header
  ...
  test "current users posts to a question id that doesn't exist" do
    post vote_api_question_url(id: 0, yes_vote: false), headers: @header
    assert_response :unprocessable_entity
  end

  test "posting with no auth header" do
    question = questions(:one)
    post vote_api_question_url(id: question.id, yes_vote: false)
